...
async def set_selecs(query: CallbackQuery, command):
    ...
    if command[1] == "view":
        ...
        for rel_id in range(10):           # sc id on page
            ...
            if ALL_SC['id'][abs_id] in picked_classes:
                ...
                button = InlineKeyboardButton(text=INT2EMOJI[rel_id+1],
                                            callback_data=f"set_selecs set {command[2]} {ALL_SC['id'][abs_id]}")  # my_selecs set <slot> <sc_id>
            if rel_id < 5:
        ...
        go_back = "func set_selecs True"
        next_page = command[3] if command[3]+1 >= len(ALL_SC['id'])//10 else len(ALL_SC['id'])//10
        button_row3.append(InlineKeyboardButton(text="⬅️", callback_data=f"set_selecs view {command[2]} {prev_page}"))
        button_row3.append(InlineKeyboardButton(text="❌", callback_data=go_back))
        button_row3.append(InlineKeyboardButton(text="➡️", callback_data=f"set_selecs view {command[2]} {next_page}"))
        ikm.add(*button_row3)
    ...
    # my_selecs set <slot> <sc_id>
        ...
        conn.execute(f"UPDATE users\
                        SET selec_class{command[2]} = {int(command[3])}\
                        WHERE id = {query.from_user.id}")
        conn.commit()
        query.data = "func set_selecs True"
