...
async def set_ignored(query: CallbackQuery, command: list):
    # Only week was set
    if len(command) == 2:
        msg_text = "Пару якого дня ви хочете ігнорувати?\n"
        buttons = []
        for i in range(2,6):
            msg_text += f"\n{i}. {DAYS[i]}"
    ...
    # Week and day was set
        ...
        week = int(command[1])
        day = int(command[2])
        msg_text = "Яку пару ви хочете ігнорувати? Номер відповідає розкладу\n"\
                    "\n"\
        ...
        for id in sched[week][day].keys():
            ...
            if pair_is_ignored(query.from_user.id, week, day, id):
                ...
                msg_text += f"\n{id}. {name} ({type})"
                callback_data=f"set_ignored add {week} {day} {id}"
