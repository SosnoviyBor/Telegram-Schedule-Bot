...
async def my_selecs(message: Message, edit_flag:bool):
    ...
    ikm.add(InlineKeyboardButton(text=f"1. {my_selecs_b(picked_classes[0])}", callback_data="my_selecs view 1 0"),  # my_selecs view <slot> <page>
            InlineKeyboardButton(text=f"2. {my_selecs_b(picked_classes[1])}", callback_data="my_selecs view 2 0"),
            InlineKeyboardButton(text=f"3. {my_selecs_b(picked_classes[2])}", callback_data="my_selecs view 3 0"))
    msg_text = "Ваші вибіркові предмети наступні:\n\n"\
            f"1. <code>{my_selecs_m(picked_classes[0])}</code>\n"\
            f"2. <code>{my_selecs_m(picked_classes[1])}</code>\n"\
            f"3. <code>{my_selecs_m(picked_classes[2])}</code>\n\n"\
